...
export function extractTestsFromSkill(skill: SkillReport): ExtractedSkill {
  if ("FullSkillReport" in skill) {
    return {
      skillName: skill.FullSkillReport.name,
      tests: skill.FullSkillReport.tests,
      breakdown: null,
    };
  }
  return {
    skillName: skill.BreakdownSkillReport.name,
    tests: null,
    breakdown: skill.BreakdownSkillReport.breakdown,
  };
}
...
export function getAllTestsFromDetails(
  ...
  return details.skills.map(extractTestsFromSkill);
}
